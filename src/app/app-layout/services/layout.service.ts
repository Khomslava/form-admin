...
import { eAppDirections, SESSION_KEY_APP_DIRECTION, tAppDirections } from '../../shared/consts/storage.consts';
...
export class LayoutService {
  ...
  constructor(
    private router: Router, private activatedRoute: ActivatedRoute, private storage: SessionStorageService
  ) {
    // Set direction from session if set.
    const sessionDirection = this.storage.retrieve(SESSION_KEY_APP_DIRECTION);
    const direction: tAppDirections =
      sessionDirection && sessionDirection === eAppDirections.RTL ? eAppDirections.RTL : eAppDirections.LTR;
  ...
  toggleDirection(): void {
    const direction: tAppDirections =
      this.direction$.getValue() === eAppDirections.LTR ? eAppDirections.RTL : eAppDirections.LTR;
